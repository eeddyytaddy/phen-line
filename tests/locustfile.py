...
class _PatchedWriter(_stats.StatsCSVFileWriter):
    def __init__(self, env, base):
        ...
        self.stats_history_file.seek(0); self.stats_history_file.truncate(0)
...
def _auto_stop(env, **_):
    if not SENTINEL.exists():                     # 第一次才需要排程自動停止
...
class LineBotUser(HttpUser):
    ...
    def _post(self, name):
        ...
        ev = payload["events"][0]
        ev.update(replyToken=str(uuid.uuid4()),
