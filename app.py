...
def handle_message_event(ev, uid, lang, replyTK):
    ...
    # 0.5) 如使用者尚未完成資料收集就輸入功能指令，提示先收集資料
    if msgType == 'text' and stage != 'ready':
        command_keywords = {
            '收集資料', '景點人潮', '景點人潮(crowd analyzer)', '行程規劃', '行程規劃(itinerary planning)',
            '景點推薦', '景點推薦(attraction recommendation)', '永續觀光', '永續觀光(sustainable tourism)',
            '一般景點推薦', '一般景點推薦(general recommendation)', '附近搜尋', '附近搜尋(nearby search)',
            '租車', '租車(car rental information)', 'data collection', 'collect data', 'crowd analyzer',
            'crowd analysis', 'crowd info', 'itinerary planning', 'plan itinerary', 'attraction recommendation',
            'recommend spot', 'sustainable tourism', 'general recommendation', 'nearby search',
            'car rental information', 'car rental', '1', '2', '2-1', '2-2', '3', '4', '5', '6'
        }
        keyword_map = {
            '餐廳': 'restaurants', '停車場': 'parking', '風景區': 'scenic spots', '住宿': 'accommodation'
        }
        if low in command_keywords or text in keyword_map or low in set(keyword_map.values()):
